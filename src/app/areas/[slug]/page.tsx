...
export default async function AreaDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!area) notFound();
  const faqs = [
    { q: `${area.name} 오피스텔도 방문 가능한가요?`, a: "건물 출입 규정과 주차 가능 여부에 따라 달라질 수 있습니다. 예약 전 상세 주소 확인 후 안내드립니다." },
        ...
        <div className="container grid gap-8 lg:grid-cols-[1fr_340px]">
          <div>
            ...
            <div className="mt-10 grid gap-5 md:grid-cols-2">
              <Card><h2 className="text-xl font-bold">주요 방문 가능 구역</h2><p className="mt-3 text-sm leading-7 text-muted-foreground">{area.zones.join(", ")}</p></Card>
